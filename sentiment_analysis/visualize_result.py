...
def calculate_max_percentage_drop_date(df_merged, column):
...
def plot_coin(ax, df_merged, sentiment_column, max_change_date, coin_name, title: str, column='Close'):
    ...
    ax.axvline(max_change_date, color='red', linestyle='--', linewidth=2, label='Max Percentage Price Drop Date')
    ax.legend(loc='upper left', fontsize=12)

    ax.annotate('Max Percentage Price Drop Date',
...
def visualize_results(folder, pos_df, neg_df, price_df, article_count_df):
    ...
    # Calculate the date of maximum percentage drop in price
    max_percentage_drop_date_pos = calculate_max_percentage_drop_date(pos_merged, 'Close')
    max_percentage_drop_date_neg = calculate_max_percentage_drop_date(neg_merged, 'Close')
    max_percentage_drop_date_articles = calculate_max_percentage_drop_date(article_count_merged, 'Close')

    # Plot price and positive sentiment
    fig, ax = plt.subplots(figsize=(12, 8))
    plot_coin(ax, pos_merged, 'Positive Sentiment', max_percentage_drop_date_pos, subfolder_name, 'Positive Sentiment')
    plt.savefig(os.path.join(folder, f'{subfolder_name}_price_pos.png'))
    plt.close()

    # Plot price and negative sentiment
    fig, ax = plt.subplots(figsize=(12, 8))
    plot_coin(ax, neg_merged, 'Negative Sentiment', max_percentage_drop_date_neg, subfolder_name, 'Negative Sentiment')
    plt.savefig(os.path.join(folder, f'{subfolder_name}_price_neg.png'))
    plt.close()

    # Plot price and daily article count
    fig, ax = plt.subplots(figsize=(12, 8))
    plot_articles_and_price(ax, article_count_merged, max_percentage_drop_date_articles, subfolder_name)
